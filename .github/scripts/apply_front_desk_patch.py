...
def sub(path: str, pattern: str, replacement: str, flags: int = re.S) -> None:
    ...
    updated, count = regex.subn(lambda _: replacement, text, count=1)
    if count != 1:
...
# ---------------------------------------------------------------------------
# PUBLIC ADMISSION — stale drafts/config changes + useful typed failures.
# ---------------------------------------------------------------------------
p = "src/components/forms/AdmissionForm.tsx"
replace(
    p,
...
sub(
    p,
    r'''\{serverError \? \(\s*<p role="alert" className="field-error">\s*\{t\("errors\.generic"\)\}\s*</p>\s*\) : null\}''',
    '''{serverError ? (
...
for path, additions in {
    "messages/en.json": {
        "rateLimited": "Too many attempts just now. Wait a few minutes, then try again.",
        "security": "The security check expired or could not be verified. Wait for it to refresh, then try again.",
        "serverValidation": "Some saved form details are no longer valid. Go back once, reselect the course and timing, then submit again.",
        "service": "The application service is temporarily unavailable. Your form is still saved on this device; please try again shortly.",
        "supportRef": "Support reference: {ref}.",
    },
    "messages/gu.json": {
        "rateLimited": "હમણાં બહુ attempts થયા છે. થોડા મિનિટ પછી ફરી try કરો.",
        "security": "Security check expire થયું છે અથવા verify થયું નથી. તે refresh થાય પછી ફરી try કરો.",
        "serverValidation": "Formમાં saved થયેલી કેટલીક વિગતો હવે valid નથી. એક વાર પાછળ જઈ course અને timing ફરી પસંદ કરીને submit કરો.",
        "service": "હમણાં application service ઉપલબ્ધ નથી. તમારું form આ device પર saved છે; થોડી વાર પછી ફરી try કરો.",
        "supportRef": "Support reference: {ref}.",
    },
}.items():
    data = json.loads(read(path))
    data["admissionForm"]["errors"].update(additions)
    write(path, json.dumps(data, ensure_ascii=False, indent=2) + "\n")


# ---------------------------------------------------------------------------
# FEES DOMAIN — daily payment never accepts the agreement amount from client.
# ---------------------------------------------------------------------------
p = "src/lib/admin/fees.ts"
marker = "/* --------------------------- the agreement -------------------------------- */"
text = read(p)
if text.count(marker) != 1:
    raise SystemExit("fees agreement marker missing")
addition = r'''export type PaymentEntryInput = {
  enrollmentId: number;
  received: number;
...
p = "src/app/admin/(console)/fees/actions.ts"
replace(
    p,
...
sub(
    p,
    r'''export async function addFeeRecordAction\(_prev: FeeState, formData: FormData\): Promise<FeeState> \{.*?\n\}\n\n(?=/\*\*\n \* Changes what an EXISTING student agreed to pay\.)''',
    r'''export async function addFeeRecordAction(_prev: FeeState, formData: FormData): Promise<FeeState> {
...
p = "src/app/admin/(console)/fees/FeeForm.tsx"
sub(
    p,
    r'''export function FeeEntryForm\(.*?\n\}\n\n(?=function Field\()''',
    r'''export function FeeEntryForm({
  enrollmentId,
  discount,
  dueDate,
...
# Fee copy — English and Gujarati parity.
p = "src/lib/admin/fees-copy.ts"
replace(
    p,
...
# Fees page — status filter and urgent balances first.
p = "src/app/admin/(console)/fees/page.tsx"
replace(
    p,
...
sub(
    p,
    r'''  const cards = enrollments\.map\(\(enrollment\) => \{.*?  const totalDue = cards\.reduce\(\(sum, card\) => sum \+ card\.summary\.balance, 0\);''',
...
sub(
    p,
    r'''      <form method="get" className="toolbar mt-3.*?</form>''',
...
sub(
    p,
    r'''              \{canManage \? \(\n                <details className="border-t border-rule pt-5">\n                  <summary className="cursor-pointer font-semibold">\{copy\.recordPayment\}</summary>.*?              \) : null\}\n\n              \{canManage \? \(\n                <details className="border-t border-rule pt-5">''',
...
# ---------------------------------------------------------------------------
# DIRECT ADMISSION — per-student agreement + first offline receipt.
# ---------------------------------------------------------------------------
p = "src/app/admin/(console)/students/StudentForms.tsx"
replace(
    p,
...
sub(
    p,
    r'''export function DirectAdmissionForm\(.*?\n\}\n\n(?=export function ConvertEnquiryForm)''',
    r'''export function DirectAdmissionForm({ batches, copy, feeCopy }: { batches: BatchOption[]; copy: StudentsCopy; feeCopy?: FeesCopy }) {
...
p = "src/app/admin/(console)/students/actions.ts"
replace(
    p,
...
# Student page fee discoverability + course defaults in walk-in form.
...
# Mobile priorities + Today shortcut simplification.
p = "src/app/admin/(console)/layout.tsx"
replace(
    p,
...
p = "src/app/admin/(console)/page.tsx"
sub(
    p,
    r'''  const quickActions = \[.*?  \]\.filter\(\(item\): item is \{ href: string; label: string \} => item !== null\);''',
    '''  const quickActions = [
...
Path("tests/front-desk-fees-ux.test.ts").write_text(r'''import { describe, expect, it } from "vitest";
...
# Durable memory for the operational decision; no architecture/provider change.
...
"""
if "### Front-desk fee workflow (2026-09-03)" not in text:
