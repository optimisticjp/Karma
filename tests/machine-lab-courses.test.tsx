...
describe("the course index", () => {
  it("reads the same Console-filtered catalogue as the homepage", () => {
  ...
  it("shows all eleven source courses before Console filtering and never truncates the rendered list", () => {
...
describe("the duration and fee policy", () => {
  it("has exactly one course with verified seed operational facts", () => {
    expect(VERIFIED_COURSE_OPERATIONS).toHaveLength(1);
    expect(VERIFIED_COURSE_OPERATIONS[0].slug).toBe(EMCAD_DAHAO_SLUG);
  });

  it("leaves the other ten with no duration in the source catalogue", () => {
    for (const course of courses) {
  ...
  it("says 'ask the studio' rather than guessing, for courses without facts", () => {
    for (const cat of [en, gu]) {
...
describe("every course page is specific", () => {
  ...
  it("links each course to the Machine Notes that answer its questions", () => {
    expect(read("src/components/kds/courses/CourseBatches.tsx")).toContain("notesForCourse(course.slug)");
