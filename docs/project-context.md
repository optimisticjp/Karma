...
**B2B Studio (Phase 8).** `<StudioRail>` renders REFERENCE → DIGITISING →
SAMPLE → CORRECTION → MACHINE-READY on the same `<ProductionRail>` as the
homepage — the reuse the component's stages-as-a-prop design was for. Those
stages carry **canonical stitch marks, not photo slots**: the 32-shot list
covers the school, not the commercial pipeline, so `RailStage.photoId` is
optional and a mark stands where no photograph is briefed. The page still
publishes **no turnaround, no file format and no price** — none is confirmed —
...
**And three from Phase 8:**

4. **The message catalogue may carry nothing the site does not render.** The
   rebuild left 249 dead leaves — 22% of the catalogue, 182 of them the copy of
   a homepage rebuilt in Phase 3. Dead copy still gets translated, still gets
   reviewed against the factual rules, and still passes the parity check, so
   the parity check looks like coverage while covering nothing; it also drifts
   out of step with the string actually on screen. `tests/kds-copy-seo.test.ts`
   scans for orphans in both directions. Public catalogue: **719 leaves**;
   Console: 198.
5. **Breadcrumbs are localized, home crumb included.** Twelve second-level
   routes emit a `BreadcrumbList` through `<PageCrumbs>`, labelled from a
   `crumbs` namespace of its own. The two deep trails that already existed were
   naming their pages in English on the Gujarati site; `breadcrumbSchema()`
   now takes the home label rather than hardcoding "Home".
6. **A route is in the sitemap exactly when its page does not opt out of the
   index.** `/terms` was being submitted while its own metadata set `noIndex`.
